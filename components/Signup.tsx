...
interface SignupProps {
  onSignup: (nom: string, email: string, pass: string) => void;
  isLoading?: boolean;
  error?: string | null;
  onClearError?: () => void;
}

const Signup: React.FC<SignupProps> = ({ onSignup, isLoading, error, onClearError }) => {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 p-4 text-slate-100">
      <div className="w-full max-w-md bg-slate-900 rounded-3xl shadow-2xl p-8 border border-slate-800 relative overflow-hidden">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-slate-100">Nouvelle Société</h2>
          <p className="text-slate-400 mt-2 text-sm">Créez votre espace de gestion centralisé</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Nom de l'entreprise</label>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800 text-center">
          <p className="text-sm text-slate-400">
            Déjà inscrit ? <Link to="/login" className="text-blue-500 font-bold hover:text-blue-400 transition-colors">Se connecter</Link>
          </p>
        </div>
      </div>
    </div>
